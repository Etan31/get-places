...
const CONCURRENCY = Math.max(1, Number(process.env.SCRAPER_CONCURRENCY));
...
export async function runPlaceScrape(
  ...
  try {
    ...
    onEvent({
      type: "status",
      message: `Searching Google Maps for "${keyword} in ${city}"...`,
    });
    ...
    return await scrapePlacesConcurrently(context, places, {
      city,
      category,
      keyword,
      limit,
...
async function scrapePlacesConcurrently(
  context,
  places,
  { city, category, keyword, limit, onEvent, isCancelled },
) {
  const rows = [];
  ...
  async function worker() {
    ...
    try {
      while (!isCancelled() && committed < limit) {
        ...
        const webSignals = details.website
          ? await scrapeWebsiteSignals(context, details.website)
          : {};
...
async function collectPlaces(page, query, limit, isCancelled = () => false) {
  ...
  for (
    let attempt = 0;
    attempt < 24 && places.size < limit && !isCancelled();
    attempt += 1
  ) {
...
async function scrapePlaceDetails(page, url) {
  ...
  return page.evaluate(() => {
    ...
    return {
      ...
      location: getByDataId("address")
        .replace(/^Address:\s*/i, "")
        .trim(),
      number: (
        phoneNode?.getAttribute("aria-label") ||
...
async function scrapeWebsiteSignals(context, websiteUrl) {
  ...
  try {
    ...
    for (const url of urlsToVisit) {
      await page
        .goto(url, { waitUntil: "domcontentloaded", timeout: 8_000 })
        .catch(() => {});
...
async function getWebsitePagesToCheck(page, websiteUrl) {
  ...
  await page
    .goto(normalized, { waitUntil: "domcontentloaded", timeout: 10_000 })
    .catch(() => {});
